...
void __attribute__ ((noinline)) FullyConnectedPerChannel(
  ...
  for (int b = 0; b < batches; ++b) {
    for (int out_c = 0; out_c < output_depth; ++out_c) {
      ...
      while (1) {
        ...
        int internal_input_offset = b * accum_depth + d;
        int internal_filter_offset = out_c * accum_depth + d;

        int32_t in_val_1  = (int32_t) input_data[internal_input_offset];
        int32_t fil_val_1 = (int32_t) filter_data[internal_filter_offset];
        acc += fil_val_1 * (in_val_1 + input_offset);
        ...
        acc += fil_val_2 * (in_val_2 + input_offset);
        ...
        acc += fil_val_3 * (in_val_3 + input_offset);
        ...
        acc += fil_val_4 * (in_val_4 + input_offset);
...
void __attribute__ ((noinline)) FullyConnectedPerChannel(
  ...
  for (int b = 0; b < batches; ++b) {
    for (int out_c = 0; out_c < output_depth; ++out_c) {
      ...
      acc = std::max(acc, output_activation_min);
      acc = std::min(acc, output_activation_max);
      output_data[out_c + output_depth * b] = static_cast<int16_t>(acc);
    }
  }
  #ifdef MEASURE_FC
  disable_counter();
...
void __attribute__ ((noinline)) FullyConnected(
...
void __attribute__ ((noinline)) FullyConnectedWithPackedInt4Weights(
    const FullyConnectedParams& params, const RuntimeShape& input_shape,
    const int8_t* input_data, const RuntimeShape& filter_shape,
