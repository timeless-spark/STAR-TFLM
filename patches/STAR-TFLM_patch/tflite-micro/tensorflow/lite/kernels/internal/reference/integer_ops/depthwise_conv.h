...
void __attribute__ ((noinline)) DepthwiseConvPerChannel(
            ...
            for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
              for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                ...
                const bool is_point_inside_image =
                    ...
                    (in_y < input_height);
                if (is_point_inside_image) {
                  int int_in_channel = in_channel;

                  int internal_input_offset = Offset(input_shape, batch, in_y, in_x, int_in_channel);
                  int internal_filter_offset = Offset(filter_shape, 0, filter_y, filter_x, output_channel);

                  int32_t in_val_1  = (int32_t) input_data[internal_input_offset];
                  int32_t fil_val_1 = (int32_t) filter_data[internal_filter_offset];
                  acc_vect[0] += fil_val_1 * (in_val_1 + input_offset);

                  ++int_in_channel;

                  if (int_in_channel >= input_depth)
                    break;

                  int32_t in_val_2  = (int32_t) input_data[internal_input_offset+1];
                  int32_t fil_val_2 = (int32_t) filter_data[internal_filter_offset+1];
                  acc_vect[1] += fil_val_2 * (in_val_2 + input_offset);
                  ...
                  int32_t in_val_3  = (int32_t) input_data[internal_input_offset+2];
                  int32_t fil_val_3 = (int32_t) filter_data[internal_filter_offset+2];
                  acc_vect[2] += fil_val_3 * (in_val_3 + input_offset);
                  ...
                  int32_t in_val_4  = (int32_t) input_data[internal_input_offset+3];
                  int32_t fil_val_4 = (int32_t) filter_data[internal_filter_offset+3];
                  acc_vect[3] += fil_val_4 * (in_val_4 + input_offset);
                }
              }
            }

            int actual_out_channel;
            for(int i = 0; (i < 4) && ((actual_out_channel = output_channel + i * depth_multiplier) < bias_depth); ++i) {
              if (bias_data) {
              ...
              output_data[internal_output_offset] = static_cast<int8_t>(acc_vect[i]);
            }
          }
          in_channel += 4;
        }
...
void __attribute__ ((noinline)) DepthwiseConvHybridPerChannel(
