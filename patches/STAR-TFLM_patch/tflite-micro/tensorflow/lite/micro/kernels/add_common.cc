...
#else
  if (output->type == kTfLiteInt8 || output->type == kTfLiteInt16 || output->type == kTfLiteInt4) {
#endif
...
#ifdef STAR
  // request scratch buffers if necessary

  if (input1->type == kTfLiteInt4) {
    int input_size =
        RuntimeShape(input1->dims->size,
                     reinterpret_cast<const int32_t*>(input1->dims->data))
            .FlatSize();
    context->RequestScratchBufferInArena(context, input_size,
                                         &data->input1_buffer_index);
  }

  if (input2->type == kTfLiteInt4) {
    int input_size =
        RuntimeShape(input2->dims->size,
                     reinterpret_cast<const int32_t*>(input2->dims->data))
            .FlatSize();
    context->RequestScratchBufferInArena(context, input_size,
                                         &data->input2_buffer_index);
  }
