        ...
        for layer in self.qkeras_model.layers:
            ...
            # inputs
                ...
                if layer.__class__.__name__ in ["QConv2D", "QConv2DBatchnorm", "QDepthwiseConv2D", "QDepthwiseConv2DBatchnorm"]:
                    reminder = in_shape[3] % in_n_parall
                    if reminder != 0:
                        in_shape[3] += in_n_parall - reminder
                else:
                    reminder = in_shape[1] % in_n_parall
                    if reminder != 0:
                        in_shape[1] += in_n_parall - reminder
            ...
            # weights
                ...
                if layer.__class__.__name__ in ["QConv2D", "QConv2DBatchnorm", "QDepthwiseConv2D", "QDepthwiseConv2DBatchnorm"]:
                    ...
                    reminder = weight_shape[3] % in_n_parall
                    if reminder != 0:
                        original_channels = weight_shape[3]
                        weight_shape[3] += in_n_parall - reminder
                        for out_ch in range(weight_shape[0]):
                            for H_i in range(weight_shape[1]):
                                for W_i in range(weight_shape[2]):
                                    insert_pos = out_ch*(weight_shape[1]*weight_shape[2]*weight_shape[3])+H_i*(weight_shape[2]*weight_shape[3])+W_i*(weight_shape[3])+original_channels
                                    for miss_0 in range(in_n_parall-reminder):
                                        weight_buffer.insert(insert_pos, 0)
                else:
                    weight_buffer = np.moveaxis(weight_buffer, 1, 0).flatten().tolist()
                    reminder = weight_shape[1] % in_n_parall
                    if reminder != 0:
                        original_in_neurs = weight_shape[1]
                        weight_shape[1] += in_n_parall - reminder
                        for out_neurs in range(weight_shape[0]):
                            insert_pos = out_neurs*(weight_shape[1])+original_in_neurs
                            for miss_0 in range(in_n_parall-reminder):
                                weight_buffer.insert(insert_pos, 0)

                tmp_weight_buffer = []
                if in_n_parall == 8:
                    if layer.__class__.__name__ in ["QConv2D", "QConv2DBatchnorm", "QDense", "QDenseBatchnorm"]:
                        for pos, group in enumerate(split_list(weight_buffer, 4)):
                            tmp_weight_buffer[4*pos:4*(pos+1)] = group[::-1]
                    else:
                        tmp_weight_buffer = weight_buffer
                elif in_n_parall == 4:
                    if layer.__class__.__name__ in ["QConv2D", "QConv2DBatchnorm", "QDense", "QDenseBatchnorm"]:
                        for pos, group in enumerate(split_list(weight_buffer, 2)):
                            tmp_weight_buffer[2*pos:2*(pos+1)] = group[::-1]
                    else:
                        tmp_weight_buffer = weight_buffer
                # elif in_n_parall == 2:
                    # if layer.__class__.__name__ in ["QConv2D", "QConv2DBatchnorm", "QDense", "QDenseBatchnorm"]:
